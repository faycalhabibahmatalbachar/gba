      ...
      >
        {!user ? null : q.isLoading ? (
          ...
          <div className="space-y-4">
            ...
            {isSuperProfile ? (
              <div className="rounded-xl border border-amber-500/35 bg-amber-500/5 p-4 text-sm">
                <p className="font-semibold text-foreground">Compte superadministrateur</p>
                <p className="text-xs text-muted-foreground mt-1">
                  Les indicateurs commerce (LTV, panier, funnel) ne s’appliquent pas. Priorité : gouvernance (rôles, sécurité,
                  devices).
                </p>
                <div className="mt-3 grid grid-cols-2 gap-3 text-xs">
                  <div>
                    <p className="text-muted-foreground">Sessions / devices</p>
                    <p className="font-medium tabular-nums">{String(q.data?.stats?.device_tokens_count ?? 0)}</p>
                  </div>
                  <div>
                    <p className="text-muted-foreground">Dernière connexion</p>
                    <p className="font-medium">
                      {p.last_sign_in_at
                        ? formatDistanceToNow(new Date(String(p.last_sign_in_at)), { addSuffix: true, locale: fr })
                        : '—'}
                    </p>
                  </div>
                </div>
            ...
            <Tabs value={tabVal} onValueChange={setTabVal}>
              <TabsList className="flex flex-wrap h-auto gap-1">
                ...
                {!isSuperProfile ? <TabsTrigger value="comportement">Comportement</TabsTrigger> : null}
